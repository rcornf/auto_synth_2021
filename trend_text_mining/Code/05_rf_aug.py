...
'''Python code to fit RF classifiers using lpi texts and trends.
	Testing various augmentation approaches.
'''
...
# K-fold cv with text augmentation
def cv_skl_eda(k, df, seed, class_col, aug_bool, alpha, aug_n, sent_strp_bool, 
	sp_rm_bool, loc_rm_bool, incl_varied_bool):
	out_ls = []
...
# spec_df_.to_csv("../Data/rf_aug_specs.csv", index = False)
...
for i in range(spec_df.shape[0]):
	...
	# Stemming
	tmp_dat["TA"] = tmp_dat.TA.apply(ps().stem_sentence)

	out_df = cv_skl_eda(10, tmp_dat, cv_spec.seed, cv_spec.class_col,

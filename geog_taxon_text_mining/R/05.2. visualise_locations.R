...
map_lpi_geo = ggplot() + 
  geom_polygon(aes(x = long, y = lat, group = group), fill = "darkgrey", data = map_base_fort, colour = "grey", size = 0.1) +
...
# set up data labels
heading_labels_auto <- data.frame(x = c(8, 8), y = c(1, 2), labels = c("LPD", "Automated"))
...
# set up data labels
heading_labels_auto <- data.frame(x = c(10, 10), y = c(1, 2), labels = c("MLD", "Automated"))
...
# set up data labels
heading_labels_auto <- data.frame(x = c(4,4), y = c(1, 2), labels = c("LPD", "MLD"))
...
ggsave("outputs/geography.png", width = 12, height = 10, dpi = 300)
